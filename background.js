...
async function getSelectedTextFromActiveTab() {
  try {
    ...
    if (
      ...
      console.log(
        "[Background] Active tab is not a webpage or no active tab found."
      );
      return "";
    }
...
chrome.action.onClicked.addListener(async (tab) => {
  console.log(
    "[Background] Action icon clicked on Tab ID:",
    tab.id,
    "URL:",
    tab.url
  );
  ...
  console.log(
    "[Background] Captured selected text (first 100 chars):",
  ...
  chrome.windows.get(tab.windowId, {}, (clickedWindow) => {
    ...
    if (chrome.runtime.lastError || !clickedWindow) {
      console.warn(
        "[Background] Failed to get details of the window where icon was clicked. Using default positioning."
      );
      ...
      left =
        clickedWindow.left +
        Math.round((clickedWindow.width - windowWidth) / 2);
      top =
        clickedWindow.top +
        Math.round((clickedWindow.height - windowHeight) / 2);
    }
    ...
    if (popupWindowId !== null) {
      chrome.windows.get(popupWindowId, {}, (existingWindow) => {
        if (chrome.runtime.lastError) {
          ...
          console.log(
            "[Background] Previous popup window not found. Creating a new one."
          );
          ...
          console.log(
            "[Background] Existing popup window found (ID:",
            popupWindowId,
            "). Closing it now."
          );
          chrome.windows.remove(popupWindowId, () => {
            if (chrome.runtime.lastError) {
              console.error(
                "[Background] Error removing previous popup:",
            ...
            // createPopupWindow(left, top, windowWidth, windowHeight);
            console.log(
              "[Background] Previous popup closed. A new one will not be opened automatically by this logic block."
            );
          });
        }
      });
  ...
  chrome.windows.create(
    ...
    (win) => {
      if (win) {
        ...
        console.log(
          "[Background] Popup window created with ID:",
          popupWindowId
        );
      } else {
        console.error(
          "[Background] Failed to create popup window:",
          chrome.runtime.lastError?.message
        );
...
chrome.windows.onRemoved.addListener((windowId) => {
  if (windowId === popupWindowId) {
    console.log(
      "[Background] Monitored popup window (ID:",
      windowId,
      ") was closed by user or other means."
    );
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "getSelectedText") {
    console.log(
      "[Background] Popup requested selected text. Sending (first 100 chars):",
      lastSelectedText.substring(0, 100)
    );
  ...
  if (message.action === "updateSelectedText") {
    ...
    console.log(
      "[Background] Received 'updateSelectedText' from popup, new text:",
