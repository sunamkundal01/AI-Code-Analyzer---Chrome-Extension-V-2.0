document.addEventListener("DOMContentLoaded", () => {
  ...
  const spaceComplexityValueEl = document.getElementById(
    "spaceComplexityValue"
  );
  const complexityDirectOutputEl = document.getElementById(
  ...
  chrome.runtime.sendMessage({ action: "getSelectedText" }, (response) => {
    if (chrome.runtime.lastError) {
      console.error(
        "[Popup] Error requesting selected text from page:",
        chrome.runtime.lastError.message
      );
      currentSelectedCodeFromPage = "";
    } else if (response && typeof response.text !== "undefined") {
      console.log(
        "[Popup] Received initial selected text from page (first 100 chars):",
        response.text.substring(0, 100)
      );
      currentSelectedCodeFromPage = response.text;
    } else {
      console.error(
        "[Popup] Invalid response when requesting selected text from page."
      );
      currentSelectedCodeFromPage = "";
    }
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === "updateSelectedText") {
      console.log(
        "[Popup] Received updated selected text from page (first 100 chars):",
  ...
  async function getCodeForAnalysis() {
    ...
    if (codeToAnalyze.length < MIN_CODE_LENGTH_FOR_SELECTION) {
      console.log(
        `[Popup] Page selection is short or empty (${codeToAnalyze.length} chars). Attempting to read from clipboard.`
      );
      ...
      try {
        ...
        if (clipboardText && clipboardText.trim() !== "") {
          console.log(
            "[Popup] Successfully read from clipboard (first 100 chars):",
          ...
          resultsEl.innerHTML =
            "<p>Using code from clipboard for this analysis...</p>";
          resultsEl.className = "result-content"; // Reset class
          resultsEl.style.display = "block";
          complexityDirectOutputEl.style.display = "none";
        } else {
          console.log(
            "[Popup] Clipboard is empty or contains only whitespace. Using page selection (if any)."
          );
      ...
      console.log(
        `[Popup] Using code selected from page (${codeToAnalyze.length} chars).`
      );
    }
    console.log(
      `[Popup] Code For Analysis (Source: ${source}, Length: ${
        codeToAnalyze.length
  ...
  async function callGeminiApi(code, promptType) {
    if (!geminiApiKey) {
      resultsEl.innerHTML =
        "Gemini API Key is not set. Please set it in the extension options.";
      resultsEl.className = "result-content error";
      resultsEl.style.display = "block";
      complexityDirectOutputEl.style.display = "none";
      return;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.warn(
            "[Popup] Could not parse error response as JSON:",
            parseError
          );
        }
        resultsEl.innerHTML = `Error from Gemini API: ${errorMessage}`;
        resultsEl.className = "result-content error";
        resultsEl.style.display = "block";
        return;
      }
      ...
      if (data.candidates && data.candidates[0]?.content?.parts?.[0]?.text) {
        ...
        if (promptType === "complexityOnly") {
          ...
          timeComplexityValueEl.textContent = lines[0]
            ? lines[0].trim()
            : "N/A";
          spaceComplexityValueEl.textContent = lines[1]
            ? lines[1].trim()
            : "N/A";
          if (lines.length > 2) {
            // If a third line exists (e.g. for stack space)
        ...
        resultsEl.innerHTML = `Blocked by API: ${
          data.promptFeedback.blockReason.reason || "Unknown"
        }. ${data.promptFeedback.blockReason.message || ""}`;
        resultsEl.className = "result-content error";
        resultsEl.style.display = "block";
      } else {
        resultsEl.innerHTML =
          "Could not parse response from Gemini API. Unexpected structure.";
        resultsEl.className = "result-content error";
        resultsEl.style.display = "block";
      }
    } catch (error) {
